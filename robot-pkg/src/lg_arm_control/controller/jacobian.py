...
class ArmController(Node):
    ...
    def calculate_rotation(self, vel_param):
        command0 = self.theta[0] + (self.rotK*vel_param)
        if command0>3.14:
            command0 = 3.14
        if command0<-3.14:
            command0=-3.14
    ...
    def control_loop(self):
        ...
        command0 = self.calculate_rotation(vy)

        # Optional: Clamp joint angles if needed
        # new_thetas = np.clip(new_thetas, -np.pi, np.pi)

        # Send command
        self.publish_joint_angles(command0, command1, command2, command3, gripper)
